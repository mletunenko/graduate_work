...
from sqlalchemy import table, column, UUID, String, Boolean, TIMESTAMP
...
def upgrade() -> None:
    ...
    op.add_column('profiles', sa.Column('role', sa.Enum('BASIC', 'SUBSCRIBER', 'ADMIN', name='role'), nullable=False))
...
def downgrade() -> None:
    ...
    op.bulk_insert(roles_table, [
        {"id": str(uuid.uuid4()), "title": "basic", "system_role": False, "created_at": datetime.now(timezone.utc)},
    ...
    op.create_foreign_key('fk_profiles_role_id_roles', 'profiles', 'roles', ['role_id'], ['id'], ondelete='SET NULL')
